...
* [1. Introduction](multisignature-wallet.md#1-introduction)
  * [Glossary](multisignature-wallet.md#glossary)
* [2. Install TONOS-CLI](multisignature-wallet.md#2-install-tonos-cli)
  * [2.1. Install TONOS-CLI utility](multisignature-wallet.md#21-install-tonos-cli-utility)
    * [Install compiled executable](multisignature-wallet.md#install-compiled-executable)
    * [Install through TONDEV](multisignature-wallet.md#install-through-tondev)
    * [Build from source](multisignature-wallet.md#build-from-source)
    * [Tails OS secure environment](multisignature-wallet.md#tails-os-secure-environment)
    * [A note on Windows syntax](multisignature-wallet.md#a-note-on-windows-syntax)
  * [2.2. Download contract files](multisignature-wallet.md#22-download-contract-files)
  * [2.3. Configure TONOS-CLI environment](multisignature-wallet.md#23-configure-tonos-cli-environment)
* [3. Create Wallet](multisignature-wallet.md#3-create-wallet)
  * [3.1. Create seed phrases and public keys for all custodians](multisignature-wallet.md#31-create-seed-phrases-and-public-keys-for-all-custodians)
    * [3.1.1. Create wallet seed phrase](multisignature-wallet.md#311-create-wallet-seed-phrase)
    * [3.1.2. Generate public key](multisignature-wallet.md#312-generate-public-key)
  * [3.2. Generate deployment key pair file](multisignature-wallet.md#32-generate-deployment-key-pair-file)
  * [3.3. Generate wallet address](multisignature-wallet.md#33-generate-wallet-address)
  * [3.4. Send tokens to the new address from another wallet](multisignature-wallet.md#34-send-tokens-to-the-new-address-from-another-wallet)
  * [3.5. Deploy wallet (set custodians)](multisignature-wallet.md#35-deploy-wallet-set-custodians)
    * [3.5.1. Deploy the wallet to blockchain](multisignature-wallet.md#351-deploy-the-wallet-to-blockchain)
    * [3.5.2. Check that the wallet is active](multisignature-wallet.md#352-check-that-the-wallet-is-active)
    * [3.5.3. Request the list of custodian public keys from the blockchain](multisignature-wallet.md#353-request-the-list-of-custodian-public-keys-from-the-blockchain)
* [4. Manage Wallet](multisignature-wallet.md#4-manage-wallet)
  * [4.1. Select blockchain network](multisignature-wallet.md#41-select-blockchain-network)
  * [4.2. Convert tokens to nanotokens](multisignature-wallet.md#42-convert-tokens-to-nanotokens)
  * [4.3. Check wallet balance and status](multisignature-wallet.md#43-check-wallet-balance-and-status)
    * [4.3.1. Check wallet balance and status with TONOS-CLI](multisignature-wallet.md#431-check-wallet-balance-and-status-with-tonos-cli)
    * [4.3.2. Check wallet balance and status in the blockchain explorer](multisignature-wallet.md#432-check-wallet-balance-and-status-in-the-blockchain-explorer)
  * [4.4. List custodian public keys](multisignature-wallet.md#44-list-custodian-public-keys)
  * [4.5. List transactions awaiting confirmation](multisignature-wallet.md#45-list-transactions-awaiting-confirmation)
  * [4.6. Create transaction online](multisignature-wallet.md#46-create-transaction-online)
    * [4.6.1. Alternative command to create transaction online](multisignature-wallet.md#461-alternative-command-to-create-transaction-online)
  * [4.7. Create transaction confirmation online](multisignature-wallet.md#47-create-transaction-confirmation-online)
    * [4.7.1. Alternative command to confirm transaction online](multisignature-wallet.md#471-alternative-command-to-confirm-transaction-online)
  * [4.8. Create new transaction offline](multisignature-wallet.md#48-create-new-transaction-offline)
  * [4.9. Create transaction confirmation offline](multisignature-wallet.md#49-create-transaction-confirmation-offline)
  * [4.10. Generate deploy message offline](multisignature-wallet.md#410-generate-deploy-message-offline)
  * [4.11. Broadcast previously generated message](multisignature-wallet.md#411-broadcast-previously-generated-message)
* [5. Error codes](multisignature-wallet.md#5-error-codes)
...
* Generate [seed phrases and custodian keys](multisignature-wallet.md#31-create-seed-phrases-and-public-keys-for-all-custodians)
* Pepare [deployment message](multisignature-wallet.md#411-broadcast-previously-generated-message) offline
* Prepare [new transaction](multisignature-wallet.md#48-create-new-transaction-offline) offline
* Prepare [transaction confirmation](multisignature-wallet.md#49-create-transaction-confirmation-offline) offline
...
> **Note**: Make sure you have downloaded the **raw** versions of the files. A common error when downloading from the github project page manually is to save the redirection page instead of the raw file. **Note**: [TON Surf](https://ton.surf/) uses a specialized version of the SetcodeMultisig contract. It will not be possible to manage a standard Setcode wallet in TON Surf.
...
If you skip this step and didn't install through [tondev](multisignature-wallet.md#install-through-tondev), make sure you run the utility from the utility folder:

```
./tonos-cli <command> <options>
```

1. Use the following command to set the network:
...
> **Note**: By default `tonos-cli` connects to `net.ton.dev` network. **Note**: Always run `tonos-cli` utility only from the folder where `tonos-cli.conf.json` is placed, unless you have configured a different path for the file. Refer to the [TONOS-CLI document](https://github.com/tonlabs/tonos-cli#24-override-configuration-file-location) for additional information.

1.  Use the following command to check the set network:
...
> **Note**: Seed phrases should be created for every custodian of the multisignature wallet. The seed phrase ensures access to the multisignature wallet. If lost, the custodian will no longer be able to manage the wallet. The seed phrase is unique for every custodian and should be kept secret and securely backed up (word order matters).
...
`<MultisigWallet.tvc>` - either `SafeMultisigWallet.tvc` or `SetcodeMultisigWallet.tvc` depending on the contract you have selected at step [2.2](multisignature-wallet.md#22-download-contract-files).

`<MultisigWallet.abi.json>` - either `SafeMultisigWallet.abi.json` or `SetcodeMultisigWallet.abi.json` depending on the contract you have selected at step [2.2](multisignature-wallet.md#22-download-contract-files).
...
> **Note**: Masterchain fees are significantly higher, but masterchain is required for direct staking validator wallets. Make sure to set workchain ID to `-1` for any direct staking validator wallets you are deploying: `--wc -1`. Basechain, on the other hand, is best suited for user wallets and validator wallets that are staking through a [DePool](https://github.com/tonlabs/ton-labs-contracts/tree/master/solidity/depool). The utility displays the new multisignature wallet address (Raw\_address).
...
> **Note**: Masterchain fees are significantly higher, but masterchain is required for validator wallets. Make sure to set workchain ID to `-1` for any validator wallets you are deploying: `--wc -1`. Basechain, on the other hand, is best suited for user wallets. `<deploy_seed_or_keyfile>` - can either be the seed phrase used in step **3.2** to generate the deployment key pair file or the `deploy.keys.json` file itself. If seed phrase is used, enclose it in double quotes.
...
> **Note**: at step \[3.4]]\(#34-send-tokens-to-the-new-address-from-another-wallet) of the wallet deployment procedure use `false`. `"payload"` - use "" for simple transfer. Otherwise payload is used as a body of outbound internal message.
...
> **Note**: For maximum security you may also create a transaction message on a machine without internet connection in offline mode. See section [4.8](multisignature-wallet.md#48-create-new-transaction-offline).
...
> **Note**: at step [3.4](multisignature-wallet.md#34-send-tokens-to-the-new-address-from-another-wallet) of the wallet deployment procedure use `false`. `payload` - use "" for simple transfer. Otherwise payload is used as a body of outbound internal message.
...
> **Note**: If the wallet has only one custodian, or if the number of confirmations required to perform a transaction was set to 1, this action won't be necessary. The transaction will be confirmed automatically. Example:
...
> **Note**: For maximum security you may also create a transaction confirmation message on a machine without internet connection in offline mode. See section [4.9](multisignature-wallet.md#49-create-transaction-confirmation-offline)
...
Copy the message text or scan the QR code and [broadcast](multisignature-wallet.md#411-broadcast-previously-generated-message) the message online.
...
If needed, signed deploy message can be generated without immediately broadcasting it to the blockchain. Generated message can be [broadcasted](multisignature-wallet.md#411-broadcast-previously-generated-message) later.
...
`<MultisigWallet.abi.json>` - either `SafeMultisigWallet.abi.json` or `SetcodeMultisigWallet.abi.json` depending on the contract you have selected at step [2.2](multisignature-wallet.md#22-download-contract-files). `message` – the content of the message generated by the TONOS-CLI utility during message creation. It should be enclosed in double quotes. Example:
